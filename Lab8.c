...
void Timer3A_Handler(void){
  ...
  // write this
	...
	ADCStatus = 1;           // Synchronize with other threads --> Indicate New Data
}
...
int main(void){ // this is real lab 8 main
  ...
  while(1){
    // check semaphore
    // output to LCD
		while(ADCStatus == 0){	}	// wair for new data in mailbox
		Data = ADCMail;		// read mailbox
		ADCStatus = 0;		// clear mailbox flag
		Position = Convert(Data);	// convert sample to fixed point
		ST7735_SetCursor(0,0);	
		ST7735_OutFix(Position);		// write to LCD
		ST7735_OutString(" cm");
  }
}
